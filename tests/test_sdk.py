...
class TestQRadarSessionAuth:
    ...
    def test_basic_auth_sets_auth(self):
        session = QRadarSession(host="10.155.170.100", username="admin", password="Cl34rP@sS1", verify_ssl=False)
        assert session._session.auth == ("admin", "Cl34rP@sS1")
    ...
    def test_custom_version(self):
        session = QRadarSession(host="10.155.170.100", sec_token=SEC_TOKEN, version="25.0", verify_ssl=False)
        assert session._session.headers["Version"] == "25.0"
        session.close()

    def test_missing_auth_raises(self):
        with pytest.raises(QRadarAuthError):
            QRadarSession(host="10.155.170.100")

    def test_base_url_construction(self):
        session = QRadarSession(host="10.155.170.100", sec_token=SEC_TOKEN, verify_ssl=False)
        assert session.base_url == "https://10.155.170.100/api"
        session.close()

    def test_base_url_with_scheme(self):
        session = QRadarSession(host="https://10.155.170.100", sec_token=SEC_TOKEN, verify_ssl=False)
        assert session.base_url == "https://10.155.170.100/api"
        session.close()

    def test_context_manager(self):
        with QRadarSession(host="10.155.170.100", sec_token=SEC_TOKEN, verify_ssl=False) as s:
            assert s.base_url == "https://10.155.170.100/api"
...
def test_500_raises_api_error():
    ...
    session = QRadarSession(host="10.155.170.100", sec_token=SEC_TOKEN, verify_ssl=False, max_retries=0)
...
class TestQRadarClient:
    def test_all_api_tags_accessible(self):
        client = QRadarClient(host="10.155.170.100", sec_token=SEC_TOKEN, verify_ssl=False)
        expected_tags = [
    ...
    def test_context_manager(self):
        with QRadarClient(host="10.155.170.100", sec_token=SEC_TOKEN, verify_ssl=False) as client:
...
def test_siem_get_offenses():
    rsps_lib.add(
        ...
        json=[{"id": 19251, "status": "OPEN"}],
        status=200,
    )
    client = QRadarClient(host="10.155.170.100", sec_token=SEC_TOKEN, verify_ssl=False)
    offenses = client.siem.get_siem_offenses(filter="id=19251")
    assert len(offenses) == 1
    assert offenses[0]["id"] == 19251
    client.close()
